...
if (METHOD == 'sparse_pls') {
  ...
  zero_fvars <- apply(feat[train_idx, ], 2, function(col) sum(col != 0)) / sum(train_idx) <= MAGIC_NUM
  ...
  for (opt in names(mofa_sets$modeloptions)) {
    ModelOptions[[opt]] <- mofa_sets$modeloptions[[opt]]
  }
  ...
  for (opt in names(mofa_sets$trainoptions)) {
    TrainOptions[[opt]] <- mofa_sets$trainoptions[[opt]]
  }
  ...
  weights <- lapply(views, function(view) {
    mat <- weights[[view]]
    rownames(mat) <- colnames(feat)[view_labs == view]
    t(mat)
  })
