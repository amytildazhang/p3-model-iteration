...
for (data_type in data_types) {
  ...
  dat[-1, 1] <- paste0(data_type, '_', as.data.frame(dat)[-1, 1])
...
for (dset in names(dsets)) {
  ...
  dsets[[dset]] <- dsets[[dset]][-1, c(id_col, shared_sample_ids)]
}
...
# duplicated features somehow
unique_ids <- !duplicated(feat_ids)
...
response <- as.vector(t(as.data.frame(response)[2, ]))

# add response and sample id columns and store result
combined_dat <- as.data.frame(cbind(sample_id = shared_sample_ids, cbind(combined_dat[,unique_ids], response)))
